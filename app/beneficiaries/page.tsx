...
import { getUserById, getUserBalance, getUserBeneficiaries } from '@/lib/db'
...
export default async function BeneficiariesPage() {
  ...
  const [balance, beneficiaries] = await Promise.all([
    ...
    <div className="flex min-h-screen">
      ...
      <main className="md:ml-64 flex-1 p-4 md:p-8 pt-20 md:pt-8 space-y-6 min-w-0 max-w-2xl">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-[#1A1A2E]">Beneficiaries</h1>
            <p className="text-sm text-[#6B7280]">Manage your saved recipients</p>
          </div>
        </div>
